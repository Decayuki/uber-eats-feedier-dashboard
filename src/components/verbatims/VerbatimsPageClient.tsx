...
export function VerbatimsPageClient({ initialParams }: VerbatimsPageClientProps) {
  ...
  return (
    <div className="space-y-5">
      <Header title="Analyse des verbatims" subtitle="Recherche textuelle et lecture qualitative des signaux client" />

      <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap gap-3">
          <label className="min-w-[260px] flex-1">
            <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-zinc-500">Recherche</span>
            <input
              value={search}
      ...
      <section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
        <article className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
          <h2 className="mb-3 text-base font-bold text-zinc-900">Verbatims filtrés ({filtered.length})</h2>
        ...
        <aside ref={panelRef} className="space-y-4 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
          <h2 className="text-base font-bold text-zinc-900">Insights dynamiques</h2>
